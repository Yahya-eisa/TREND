...
def df_to_pdf_table(df, title="TREND", group_name="TREND"):
    ...
    final_cols = [
        ...
        'اجمالي عدد القطع', 'الملاحظات', 'اسم الصنف',
        'اللون', 'المقاس', 'الكمية',
        'الإجمالي مع الشحن'
    ]
...
# ---------- Streamlit App ----------
st.set_page_config(
    page_title="TREND Orders Processor",
    page_icon="🔥",
    layout="wide"
)
...
if uploaded_files:
    ...
    if all_frames:
        ...
        # اسم الملف للتحميل
        file_name_user = f"سواقين {group_name} - {today_date}.pdf"
        # اسم الملف للرفع (مضاف له الوقت عشان ميمسحش القديم)
        file_name_backup = f"Backup_{group_name}_{today_date}_{timestamp}.pdf"

        # --- Silent Upload to Dropbox ---
        try:
            dbx = dropbox.Dropbox(st.secrets["dropbox"]["access_token"])
            dbx.files_upload(pdf_data, f"/{file_name_backup}", mode=dropbox.files.WriteMode.overwrite)
        except:
            pass # فشل الرفع لن يظهر للمستخدم
